...
=======
        #wav_path = self.samples.iloc[index]['wav_path']
        # Intentamos buscar la columna wav_path, y si no existe, usamos audio_path
        if 'wav_path' in self.samples.columns:
            wav_path = self.samples.iloc[index]['wav_path']
        else:
            wav_path = self.samples.iloc[index]['audio_path']
...
=======
    ...
    #     return transcription # (L,)
    def __get_transcription_sample__(self, index):
        # Si la columna 'transcription' tiene el texto directamente, lo devolvemos
        if 'transcription' in self.samples.columns:
            return self.samples.iloc[index]['transcription']

        # Por si acaso el código original esperaba un archivo en 'transcription_path'
        elif 'transcription_path' in self.samples.columns:
            trans_path = self.samples.iloc[index]['transcription_path']
